            ...
            if (err || !found) {
                ...
                database && database.collection(cacheCollectionMeta, function(err, collection) {
                    ...
                    var req = collection.findOne({ key: key }, { requests: { $slice: -1 } }, function(err, res) {
                        ...
                        if (!res || !res.requests || !res.requests[0] || !res.requests[0].value) {
                            _this.log(request, 'Cache history for ' + key + 'doesn\'t exist');
                            return callback(null, null);
                        }
                        _this.log(request, 'Returning last cache from ' + cacheCollectionMeta);
                        callback(null, {
                            statusCode: res.requests[0].statusCode,
                            value: res.requests[0].value,
                            hash: res.requests[0].hash ? res.requests[0].hash : md5(res.requests[0].value)
                        });
                    });
                });
            } else {
    ...
    set: function(request, key, value, callback) {
        ...
        database && database.collection(cacheCollectionMeta, function(err, collection) {
            ...
            if (!request.prerender.statusCode || !String(request.prerender.statusCode).match(/^2.*/)) {
                ...
                historyObj.hash = md5(typeof value === 'string' ? value : '');
            }
